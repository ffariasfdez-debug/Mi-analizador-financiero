...
# 1. Configuración de la página completa (¡ESTO DEBE IR PRIMERO!)
...
# 3. TU DICCIONARIO MAESTRO (Manteniendo intactas tus listas de activos)
mis_listas_limpias = {
    "Robótica": [
    ...
    # Aquí siguen tus otras categorías (Semiconductores, Clean Energy, etc.) si las tienes
}

# 4. DATOS DE CONTROL PARA EL PROYECTO (Precios medios y cantidades reales/simuladas)
datos_compra = {
    "NOW": {"precio_medio": 85.0, "cantidad": 10},
    # Añade aquí el resto de tus posiciones compradas para que el semáforo las detecte automáticamente
}

# =========================================================================
# 5. BLOQUE DE ANÁLISIS INDIVIDUAL (Ubicado correctamente abajo para no romper nada)
# =========================================================================
st.header("🔍 Análisis Detallado Individual")
st.write("Introduce el Ticker de la acción para generar Gráficos y Diagnósticos:")

# Selector de Ticker con persistencia fija
    ...
    try:
        ...
        if not hist.empty:
            ...
            if ticker_input in datos_compra:
                ...
                # Renderizado en pantalla de Alertas y KPIs
                st.subheader(f"{semaforo_color} Estado: {semaforo_texto}")
                st.info(f"💡 **Consejo Dinámico:** {consejo_texto}")
                st.write("---")
                
                col1, col2, col3, col4 = st.columns(4)
                with col1: st.metric("Participaciones", f"{cantidad_acciones} ud")
                with col2: st.metric("Precio Compra vs Actual", f"{precio_compra:.2f}€", f"{precio_actual:.2f}€")
                with col3: st.metric("Rendimiento (%)", f"{rendimiento_porcentaje:.2f}%")
                with col4: st.metric("Beneficio Neto Total", f"{rendimiento_absoluto:.2f}€")
            else:
                st.warning("⚠️ Activo de mercado general (Fuera de cartera). Mostrando datos de cotización estándar.")
                precio_compra = None

            # Renderizado del gráfico con línea de coste integrada
