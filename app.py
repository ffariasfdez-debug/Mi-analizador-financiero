...
with pestaña1:
    ...
    df_cartera_inteligente, caja_libre, gastado_semana = motor_bot_inteligente(
        st.session_state.listas_guardadas["Robótica Pura"], max_por_accion, tope_semanal, mercado_activo
    )
            ...
            if not datos_hist.empty and len(datos_hist) >= 200:
                ...
                if p_actual_ind > p_media_200:
                    ...
                    if p_actual_ind > p_media_50:
                        ...
                        explicacion_ind = f"El activo cotiza con fuerza por encima de sus dos medias móviles principales (50 y 200 días). Esto indica un momentum institucional impecable."
                    else:
                        diagnostico_txt = "ACUMULAR (Retroceso Técnico)"
                        explicacion_ind = f"El activo mantiene su tendencia alcista principal a largo plazo (sobre la media de 200 días), pero ha corregido a corto plazo por debajo de la de 50 días. Zona óptima de acumulación."
                else:
                    estado_ind = "🔴 TENDENCIA BAJISTA / BAJO MOMENTUM"
                    diagnostico_txt = "ESPERAR (Falta de Fuerza)"
                    explicacion_ind = f"El precio cotiza por debajo de la media institucional de 200 días. El entorno técnico es complejo, conviene esperar."
...
with pestaña3:
    ...
    with col_mod2:
        ...
        if st.button("❌ Eliminar Ticker de la Lista"):
            if ticker_eliminar in st.session_state.listas_guardadas[lista_a_revisar]:
                ...
                st.success(f"¡{ticker_eliminar} retirado con éxito de {lista_a_revisar}!")
                st.rerun()
    ...
    if st.button("💾 Guardar Nueva Lista en el Sistema"):
        if nombre_nueva_lista and tickers_nueva_lista:
            ...
            st.success(f"¡La nueva lista '{nombre_nueva_lista}' ha sido registrada con éxito!")
            st.rerun()
