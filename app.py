...
def detectar_moneda(ticker):
    """
    Detecta la moneda de un ticker de forma inteligente.
    1. Primero intenta con yfinance .info (más preciso)
    2. Si falla, usa heurística por sufijo del ticker
    3. Fallback a USD
    """
    # Heurística por sufijo del ticker (rápido, no necesita API)
    sufijos_eur = ['.AS', '.PA', '.DE', '.BR', '.MI', '.MC', '.ST', '.HE', '.CO', '.OL', '.VI', '.LS', '.IR', '.L', '.LN']
    sufijos_gbp = ['.L', '.LN']
    ...
    for sufijo in sufijos_eur:
        if ticker_upper.endswith(sufijo):
            return 'EUR'
    for sufijo in sufijos_gbp:
        if ticker_upper.endswith(sufijo):
            return 'GBP'

    # Intentar con yfinance (más preciso pero más lento)
    try:
        t = yf.Ticker(ticker)
        info = t.info
...
def obtener_info_segura(ticker):
    ...
    Retorna (target, div_yield_decimal, moneda) o (None, None, None) si falla.
    """
    try:
        t = yf.Ticker(ticker)
        info = t.info
        ...
        # Normalizar dividend yield a decimal
        if dy is not None:
            if dy > 1.0:
                dy = dy / 100.0
...
def descargar_datos_seguro(tickers, period="1y", interval=None, actions=False):
    ...
    try:
        ...
        # Si es un solo ticker, normalizar a MultiIndex
...
def extraer_precio_actual(datos_minuto, ticker, historial):
    ...
    Si el mercado está cerrado, usa el último precio del historial (no devuelve None).
    """
    # 1. Intentar desde datos de minuto
    try:
    ...
    # 2. Fallback: último precio del historial diario (siempre disponible)
    try:
        if not historial.empty:
...
def formatear_pnl(ganancia_valor, ganancia_pct, moneda='USD'):
    """Formatea el P&L con colores y símbolo de moneda correcto."""
    simbolo = simbolo_moneda(moneda)
    if ganancia_valor > 0:
        return f"🟩 +{ganancia_valor:.2f} {simbolo} (+{ganancia_pct:.2f}%)"
    elif ganancia_valor < 0:
        return f"🟥 {ganancia_valor:.2f} {simbolo} ({ganancia_pct:.2f}%)"
    else:
        return f"⬜ 0.00 {simbolo} (0.00%)"
...
def migrar_cartera_antigua(df):
    """
    Migra carteras antiguas que no tienen columna 'Moneda'.
    Detecta moneda por ticker o por el símbolo usado en Precio Entrada.
    """
    if 'Moneda' not in df.columns:
        monedas = []
        for _, fila in df.iterrows():
            ticker = fila.get('Ticker', '')
            # Si el Precio Entrada tiene €, asumimos EUR
            precio_str = str(fila.get('Precio Entrada', ''))
            if '€' in precio_str:
                monedas.append('EUR')
            elif '$' in precio_str:
                monedas.append('USD')
            else:
                monedas.append(detectar_moneda(ticker))
        df['Moneda'] = monedas
    return df
...
if "cartera_compras" not in st.session_state:
    if os.path.exists(ARCHIVO_CARTERA):
        try:
            ...
            st.session_state.cartera_compras = migrar_cartera_antigua(df_cargada)
        except:
...
with pestaña1:
    ...
    if not df_mostrar.empty:
        ...
        for _, fila in df_mostrar.iterrows():
            ...
            # Extraer precio en vivo (o último conocido si mercado cerrado)
            p_live = extraer_precio_actual(cotizaciones_vivas, t_actual, pd.DataFrame())

            # Si no hay precio en vivo, intentar con historial individual
            if p_live is None:
                try:
    ...
    if accion:
        ...
        try:
            ...
            if datos_hist.empty or len(datos_hist) < 200:
                ...
                c_i1.metric("Precio de Mercado", f"{p_actual_ind:.2f} {sym}")
