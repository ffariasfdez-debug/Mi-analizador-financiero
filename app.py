...
# =========================================================
# PESTAÑA 1: BOT MASIVO AUTOMÁTICO 30K (FILTRADO INSTITUCIONAL AVANZADO)
# =========================================================
with pestaña1:
    ...
    def motor_bot_inteligente_avanzado(lista_tickers, inversion_bloque, limite_semana, max_cupo, mercado_on):
        ...
        for tick in lista_tickers:
            try:
                ...
                if not historial.empty and len(historial) >= 200:
                    ...
                    # FILTRO 1: Tendencia Institucional (Por encima de la de 200)
                    if precio_actual > media_200:
                        ...
                        if precio_actual >= (media_30 * 0.98):
                            ...
                            if crecimiento_porcentaje >= 20.0:
                                ...
                                # DETECTAR SI PAGA DIVIDENDOS (0% ABSOLUTO)
                                ...
                                # EVALUACIÓN DE VOLUMEN INSTITUCIONAL
        ...
        if candidatas_finalistas and mercado_on:
            ...
            for _, fila in df_ordenado.iterrows():
                # Control estricto del límite de activos en cartera (Cupo)
                ...
                posiciones_compradas.append({
                    ...
                    "Crecimiento": f"🚀 {fila['Crecimiento Anual']:.1f}%",
                    "Potencial Real": f"{fila['Potencial Real']:.1f}%",
    ...
    if mercado_activo:
        ...
        st.info("🛒 Sistema Canalizado: Preselección lista. Ejecuta el bot con Wall Street abierto (15:30 a 22:00 Hora España) para comprar con precios en vivo.")
...
with pestaña2:
    ...
    # Explicación resumida y escueta de las métricas solicitadas
    st.info("""
    💡 **Guía Rápida de Métricas:**
    * **Ratio R:B (Riesgo : Beneficio):** Muestra cuánto ganas por cada euro que arriesgas hasta el suelo mínimo. **Ejemplo (1 : 3.5):** Arriesgas 1€ para ir a buscar 3,50€ de beneficio. Cuanto mayor sea el segundo número, mejor.
    * **Rendimiento Dividendo:** Filtro para verificar si la empresa premia en efectivo o retiene el capital (Puro Crecimiento 0%).
    """)
            ...
            for tick in tickers_lista:
                try:
                    ...
                    if not h.empty and len(h) >= 50:
                        ...
                        # 1. POTENCIAL REAL (ELIMINADO EL 25% FIJO)
                        target_val = ticker_info.get('targetMedianPrice')
                        if target_val is None or target_val == 0:
                            target_val = p_actual * 1.225
